...
public class ImageSplitter extends AbstractSplitter {
  ...
  /**
   * Resizes the input to the new dimensions given by getNewResolution The image
   * is extended in either the X or Y direction with extra black space to
   * maintain the correct aspect ratio for the final display array
   */
   ...
   */
  private void writeImage(BufferedImage image, String dest) throws FileNotFoundException,
      IOException {
  ...
  public void splitFile() throws Exception {
    ...
    for (int x = 0; x < cols; x++) {
      for (int y = 0; y < rows; y++) {
        BufferedImage cropped = image.getSubimage(x * screenwidth, y * screenheight, screenwidth,
            screenheight);
